...
root = Tk()
root.title("MSC Break-Fix Inventory")
...
GROUP = StringVar()
BUSINESS = StringVar()
business_choices = ['BG2','BG1','BG3','Seed Stock', 'BG4', 'BG5']
group_choices = ['Breakfix', 'BG1', 'BG3', 'BG4', 'BG5']
...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `users` (user_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT, password TEXT, privlevel INTEGER, belonging TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `BG1` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `BG5` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `BG4` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `piesiri` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `amp` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `seed` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS 'parts' (part_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT)")
    cursor.execute("SELECT * FROM `users` WHERE `username` = 'admin'")
    if cursor.fetchone() is None:
        hash = pbkdf2_sha256.encrypt("admin", rounds=2000, salt_size=16)
        cursor.execute("INSERT INTO `users` (username, password, privlevel, belonging) VALUES(?, ?, ?, ?)", ('admin', hash, 1, 'Breakfix'))
        conn.commit()
        lbl_result.config(text="Default 'admin' account created with password 'admin'.")
        call(["chflags", "hidden", dbLocation])

def get_script_path():
    return os.path.dirname(os.path.realpath(sys.argv[0]))

def Exit():
    result = tkMessageBox.askquestion('MSC Break-Fix Inventory', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("MSC Break-Fix Inventory/Account Login")
    width = 600
    height = 400
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    ...
    Home.title("MSC Break-Fix Inventory/Home")
    width = 612
    if belonging == "Breakfix":
        height = 600
    else:
        height = 300
    ...
    Title.grid(row=0, columnspan=2, padx=40)
    lbl_display = Label(Title, text="MSC Break-Fix Inventory", font=('arial', 45), fg="#BDBDBD", bg="#202020")
    lbl_display.grid(row=0, columnspan=2)
    info = "User: " + str(user_name) + "           " + "Group: " + belonging 
    lbl_info = Label(Home, text=info, font=('arial', 15), bd=18, fg="#BDBDBD", bg="#202020")
    lbl_info.grid(row=2, columnspan=2)
    if belonging == "BG1":
        btn_viewInv = Button(Home, text="View Inventory", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowBG1)
        btn_viewInv.grid(row=3, column=1)
        btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=20, highlightbackground="#202020", command=Exit2)
        btn_exit2.grid(row=6, columnspan=2)
    elif belonging == "BG3":
        btn_viewPie = Button(Home, text="View Inventory", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowPie)
        btn_viewPie.grid(row=3, column=1)
        btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=20, highlightbackground="#202020", command=Exit2)
        btn_exit2.grid(row=6, columnspan=2)
    elif belonging == "BG4":
        btn_viewTer = Button(Home, text="View Inventory", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowTer)
        btn_viewTer.grid(row=3, column=1)
        btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=20, highlightbackground="#202020", command=Exit2)
        btn_exit2.grid(row=6, columnspan=2)
    elif belonging == "Triti":
        btn_viewTri = Button(Home, text="View Inventory", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowTri)
        btn_viewTri.grid(row=3, column=1)
        btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=20, highlightbackground="#202020", command=Exit2)
        btn_exit2.grid(row=6, columnspan=2)
    else:
        btn_viewInv = Button(Home, text="BG2", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowAmp)
        btn_viewInv.grid(row=3, column=1)
        btn_viewInv = Button(Home, text="BG1", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowBG1)
        btn_viewInv.grid(row=4, column=1)
        btn_viewPie = Button(Home, text="BG3", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowPie)
        btn_viewPie.grid(row=5, column=1)
        btn_viewTer = Button(Home, text="BG4", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowTer)
        btn_viewTer.grid(row=6, column=1)
        btn_viewTri = Button(Home, text="BG5", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowTri)
        btn_viewTri.grid(row=7, column=1)
        btn_viewSee = Button(Home, text="Seed Stock", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=ShowSeed)
        btn_viewSee.grid(row=8, column=1)
        btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=20, highlightbackground="#202020", command=Exit2)
        btn_exit2.grid(row=10, columnspan=2)
    if privlevel == 2 or privlevel == 3:
        ...
        btn_changepass.grid(row=3, column=0)
    if privlevel == 1:
        btn_settings = Button(Home, text="Settings", font=('arial', 18), width=20, pady=20, highlightbackground="#202020", command=Settings)
...
def UserForm():
    ...
    lbl_belonging = Label(MidUserForm, text="Group:", font=('arial', 25), fg="#BDBDBD", bg="#202020", bd=18)
    lbl_belonging.grid(row=3)
    lbl_userpriv = Label(MidUserForm, text="Privilege Level", font=('arial', 25), fg="#BDBDBD", bg="#202020", bd=18)
    lbl_userpriv.grid(row=4)
    ...
    opt_belonging = OptionMenu(MidUserForm, BELONGING, *group_choices)
    opt_belonging.grid(row=3, column=1)
    opt_belonging.config(bg = "#202020", width=15)
    opt_priv = OptionMenu(MidUserForm, USERPRIV, *priv_choices)
    opt_priv.grid(row=4, column=1)
    ...
    btn_changepass.grid(row=5, columnspan=2, pady=20)
    btn_changepass.bind('<Return>', AddUser)

def AddUser():
    USERPRIV.set(int(USERPRIV.get()[0]))
    if NEWUSERNAME.get() == "" or PRIVLEVEL.get() == "" or BELONGING.get() == "":
        tkMessageBox.showerror('New User', 'Missing information.', icon='error')
    else:
        if NEWPASSWORD1.get() != NEWPASSWORD2.get():
            tkMessageBox.showerror('New User', 'Passwords do not match.', icon='error')
            NEWPASSWORD1.set("")
            NEWPASSWORD2.set("")
        else:
            Database()
            ...
            if fetch != None:
                tkMessageBox.showerror('Error','User alrady exists', icon="error")
                NEWUSERNAME.set("")
                NEWPASSWORD1.set("")
                NEWPASSWORD2.set("")
                USERPRIV.set("")
            else:
                hash = pbkdf2_sha256.encrypt(NEWPASSWORD1.get(), rounds=2000, salt_size=16)
                cursor.execute("INSERT INTO `users` (username, password, privlevel, belonging) VALUES(?, ?, ?, ?)", (str(NEWUSERNAME.get()), hash, int(USERPRIV.get()), str(BELONGING.get())))
                conn.commit()
                ...
                cursor.close()
                conn.close()
                tkMessageBox.showinfo('New User', 'User added successfully.', icon='info')
    ...
    if NEWPASSWORD1.get() != "" or NEWPASSWORD2.get() != "":
        if NEWPASSWORD1.get() == NEWPASSWORD2.get():
            ...
            if len(fetch) == 0:
                ...
                hash = pbkdf2_sha256.encrypt(OLDPASSWORD.get(), rounds=2000, salt_size=16)
...
def UpdateBG1():
    ...
    if BUILDING.get() == "" or ROOMNUMBER.get() == "" or LOCATION.get() == "":
        ...
        if BUILDING.get() == editSelect[1] and ROOMNUMBER.get() == str(editSelect[2]) and LOCATION.get() == editSelect[3]:
            ...
            STORAGE.set("")
            editForm.withdraw()
            tkMessageBox.showinfo('Part Edit', 'Successfully updated the part.', icon='info')
        else:
            cursor.execute("SELECT * FROM `BG1` WHERE `building` COLLATE NOCASE = ? AND `roomnumber` COLLATE NOCASE = ? AND `location` COLLATE NOCASE = ?", (BUILDING.get(), ROOMNUMBER.get(), LOCATION.get()))
            if cursor.fetchone() is not None:
                tkMessageBox.showerror('Part Edit', 'Location is already in use', icon='error')
                BUILDING.set("")
                LOCATION.set("")
                ROOMNUMBER.set("")
            else:
...
def CheckOutBG1():
    ...
    if AMOUNT.get() != 0:
        Database()
        cursor.execute("""SELECT count FROM BG1 WHERE item_id = ? """, (checkSelect[0],))
        conn.commit
        fetch = cursor.fetchall()
        if len(fetch) == 0:
            tkMessageBox.showerror('Check Out', 'Location not found',icon='error')
        else:
            ...
            if newAmount < 0:
                newAmount = 0
            cursor.execute("""UPDATE BG1 SET count = ? WHERE item_id = ? """, (newAmount, checkSelect[0]))
            conn.commit()
            tkMessageBox.showinfo('Check Out', 'Successfully updated inventory')
            addnewcheck.withdraw()
            cursor.execute("""SELECT * FROM BG1 WHERE item_id = ?""", (checkSelect[0],))
            conn.commit()
            fetch = cursor.fetchone()
            partnum = fetch[7]
            business = fetch[4]
            if newAmount <= 10:
                cursor.execute("""SELECT * FROM BG1 WHERE business = ? AND partnum = ? AND storage = ?""", (business, partnum, 'Yes'))
                conn.commit()
                fetch = cursor.fetchone()
                if fetch is None:
...
def CheckInBG1():
    ...
    if AMOUNT.get() != 0:
        ...
        fetch = cursor.fetchall()
        if len(fetch) == 0:
            tkMessageBox.showerror('Check In', 'Location not found',icon='error')
        else:
            newAmount = fetch[0]
            newAmount = newAmount[0]
            ...
            cursor.execute("""UPDATE BG1 SET count = ? WHERE item_id = ? """, (newAmount, checkSelect[0]))
            conn.commit()
            tkMessageBox.showinfo('Check In', 'Successfully updated inventory',icon='info')
            setLocation = str(checkSelect[1]) + "//" + str(checkSelect[2]) + "//" + str(checkSelect[3])
            addnewcheck.withdraw()
            cursor.close()
            conn.close()
...
def ShowAudit():
    ...
    auditnewform.title("MSC Break-Fix Inventory/Audit")
    width = 300
    height = 300
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("MSC Break-Fix Inventory/Add new")
    width = 400
    height = 700
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def NewSku():
    ...
    if VENDOR.get() == "" or PARTDESC.get() == "" or PARTNUM.get() == "" or CATEGORY.get() == "" or SUBCATEGORY.get() == "":
        tkMessageBox.showerror('Missing Info', 'Please add missing information.', icon="error")
    else:
        cursor.execute("SELECT * FROM `parts` WHERE `vendor` COLLATE NOCASE = ? AND `partnum` COLLATE NOCASE = ?", (VENDOR.get(), PARTNUM.get()))
        if cursor.fetchone() is not None:
            tkMessageBox.showerror('Part Exists', 'Part already exists in inventory', icon="info")
            VENDOR.set("")
            PARTDESC.set("")
            PARTNUM.set("")
            CATEGORY.set("")
            ...
            CATEGORY.set("")
            SUBCATEGORY.set("")
            tkMessageBox.showinfo('Created SKU', 'Successfully created new SKU', icon="info")
            ResetSku()
            skuForm.withdraw()
        conn.close()
...
def AddNewBG1():
    ...
    if BUILDING.get() == "" or ROOMNUMBER.get() == "" or LOCATION.get() == "" or BUSINESS.get() == "":
        ...
        if cursor.fetchone() is not None:
            ...
            COUNT.set("")
            STORAGE.set("")
            tkMessageBox.showinfo('Part Added','Successfully added part to inventory.', icon="info")
            addnewform.withdraw()
    conn.close()
    ResetBG1()
...
def ShowParts():
    ...
    SEARCH2.set("")
    partsform = Toplevel()
    partsform.title("MSC Break-Fix Inventory/Parts")
    width = 1200
    height = 800
...
def ShowLogs():
    global logform
    logform = Toplevel()
    logform.title("MSC Break-Fix Inventory/View Log")
    width = 1200
    height = 800
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def ViewLogs():
    ...
    btn_clearfilter.pack(side=TOP, anchor='center')
    lbl_seperate = Label(LeftViewForm, text="----------------------------------------", font=('arial', 15), fg="#BDBDBD", bg="#202020")
    lbl_seperate.pack(side=TOP, anchor='center')
...
def FullAudit():
    ...
    if fetch != None:
        ...
        tkMessageBox.showinfo('Full Audit','Full audit successfully exported.', icon="info")
    else:
        tkMessageBox.showerror('Error','No items to audit', icon="error")
    cursor.close()
    conn.close()
...
def ViewForm():
    ...
    MidViewForm.config(bg="#202020")
    if CURRENTBUS.get() != "":
        inventory_name = "Inventory/" + CURRENTBUS.get()
    else:
    ...
    if privlevel == 1:
        lbl_seperate = Label(LeftViewForm, text="----------------------------------------", font=('arial', 15), fg="#BDBDBD", bg="#202020")
        lbl_seperate.pack(side=TOP, anchor='center')
        btn_add = Button(LeftViewForm, text="Add Part", highlightbackground="#202020", command=ShowParts)
        btn_add.pack(side=TOP, padx=10, pady=10, fill=X)
    ...
    scrollbarx = Scrollbar(MidViewForm, orient=HORIZONTAL)
    scrollbary = Scrollbar(MidViewForm, orient=VERTICAL)
    tree = ttk.Treeview(MidViewForm, columns=("item_id", "Building", "RoomNum", "Location", "Business", "Vendor", "PartDesc", "PartNum", "Category", "SubCat", "Count", "Storage"), selectmode="extended", height=100, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    ttk.Style().configure("Treeview", background="#CCBFB7", foreground="#000000", fieldbackground="#CCBFB7")
    tree['show'] = 'headings'
    ...
    tree.heading('Building', text="Building",anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Building', False))
    tree.heading('RoomNum', text="Room #",anchor='center', command=lambda: \
    ...
    tree.heading('Business', text="Busines", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Business', False))
    tree.heading('Vendor', text="Vendor", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Vendor', False))
    tree.heading('PartDesc', text="Part Desc", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'PartDesc', False))
    tree.heading('PartNum', text="Part Number", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'PartNum', False))
    tree.heading('Category', text="Category", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Category', False))
    tree.heading('SubCat', text="Sub-Category", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'SubCat', False))
    tree.heading('Count', text="Count", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Count', False))
    tree.heading('Storage', text="Storage", anchor='center', command=lambda: \
                    treeview_sort_column(tree, 'Storage', False))
    tree.column('#1', stretch=NO, minwidth=40, width=60)
    ...
    tree.column('#9', stretch=NO, minwidth=40, width=260)
    tree.column('#10', stretch=NO, minwidth=40, width=200)
    tree.column('#11', stretch=NO, minwidth=40, width=60)
    tree.column('#12', stretch=NO, minwidth=40, width=60)
    tree.pack()
    if CURRENTBUS.get() == "BG1":
        DisplayDataBG1()
    elif CURRENTBUS.get() == "BG3":
...
def ConfigurationForm():
    ...
    tree2.heading('belonging', text="Group", anchor='center', command=lambda: \
                    treeview_sort_column(tree2, 'belonging', False))
    tree2.column('#1', stretch=NO, minwidth=40, width=40)
    tree2.column('#2', stretch=NO, minwidth=40, width=150)
    tree2.column('#3', stretch=NO, minwidth=40, width=60)
    tree2.column('#4', stretch=NO, minwidth=40, width=150)
    tree2.pack()
    DisplayUsers()

def DisplayUsers():
    Database()
    cursor.execute("SELECT user_id, username, privlevel, belonging FROM 'users' ORDER BY 'username' COLLATE NOCASE DESC")
    fetch = cursor.fetchall()
    for data in fetch:
...
def DisplayDataBG1():
    Database()
    cursor.execute("SELECT * FROM `BG1`")
    fetch = cursor.fetchall()
    for data in fetch:
...
def ExportLog():
    ...
    if fetch != None:
        ...
        tkMessageBox.showinfo('Log Export','Log successfully exported.', icon="info")
    else:
        tkMessageBox.showerror('Error','Log is empty', icon="error")
    cursor.close()
    conn.close()
...
def SearchAmp():
    if SEARCH.get() != "":
        ...
        Database()
        cursor.execute("SELECT * FROM `amp` WHERE `business` = ? AND `partdesc` LIKE ? OR `vendor` LIKE ?", ('BG1', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%'))
        fetch = cursor.fetchall()
        for data in fetch:
            tree.insert('', 'end', values=(data))
        cursor.close()
...
def SearchBG1():
     if SEARCH.get() != "":
        tree.delete(*tree.get_children())
        Database()
        cursor.execute("SELECT * FROM `BG1` WHERE `building` LIKE ? OR `roomnumber` LIKE ? OR `location` LIKE ? OR `vendor` LIKE ? OR `partdesc` LIKE ? OR `partnum` LIKE ? OR `category` LIKE ? or `subcategory` LIKE ?", ('%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%'))
        fetch = cursor.fetchall()
        for data in fetch:
            tree.insert('', 'end', values=(data))
        cursor.close()
...
def ClearLog():
    result = tkMessageBox.askquestion('MSC Break-Fix Log', 'Are you sure you want to clear the log?', icon="warning")
    if result == 'yes':
        Database()
...
def DeleteBG1():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('MSC Break-Fix Inventory', 'Are you sure you want to remove this part?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
            tree.delete(curItem)
            Database()
            cursor.execute("DELETE FROM `BG1` WHERE `item_id` = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            ResetBG1()
...
def DelUser():
    if not tree2.selection():
       print("ERROR")
        ...
        result2 = tkMessageBox.askquestion('MSC Break-Fix Inventory', 'Are you sure you want to delete this user?', icon="warning")
        if result2 == 'yes':
            ...
            cursor.close()
            conn.close()
            privilege = 'Privilege ' + str(selecteditem2[3])
            Log('Deleted User', selecteditem2[1], privilege)

def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("MSC Break-Fix Inventory/View Inventory")
    width = 1200
    height = 800
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def ShowBG1():
    SEND_BUSINESS.set("BG1")
    CURRENTBUS.set("BG1")
    ShowView()

def ShowPie():
    SEND_BUSINESS.set("BG3")
    CURRENTBUS.set("BG3")
    ShowView()

def ShowTer():
    SEND_BUSINESS.set("BG4")
    CURRENTBUS.set("BG4")
    ShowView()

def ShowTri():
    SEND_BUSINESS.set("BG5")
    CURRENTBUS.set("BG5")
    ShowView()

def ShowSeed():
    SEND_BUSINESS.set("Seed")
    CURRENTBUS.set("Seed")
    ShowView()

def ShowAmp():
    SEND_BUSINESS.set("BG2")
    CURRENTBUS.set("BG2")
    ShowView()
...
def Logout():
    result = tkMessageBox.askquestion('MSC Break-Fix Inventory', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        user_id = ""
        root.deiconify()
        Home.destroy()
...
def Login(event=None):
    ...
    if USERNAME.get() == "" or PASSWORD.get() == "":
        ...
        hash = pbkdf2_sha256.encrypt(PASSWORD.get(), rounds=2000, salt_size=16)
...
#========================================LABEL WIDGET=====================================
lbl_display = Label(Title, text="MSC Break-Fix Inventory", font=('arial', 45), bg="#202020", fg="#BDBDBD")
lbl_display.pack()
btn_newlogin = Button(root, text="Login", font=('arial', 18), width=10, pady=20, command=ShowLoginForm, highlightbackground="#202020")
btn_newlogin.pack()
