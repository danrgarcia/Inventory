...
def Database():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS `inventory` (item_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, building TEXT, roomnumber TEXT, location TEXT, business TEXT, vendor TEXT, partdesc TEXT, partnum TEXT, category TEXT, subcategory TEXT, count INTEGER, storage TEXT)")
...
def Home():
    ...
    lbl_info.grid(row=2)
    btn_viewInv = Button(Home, text="Inventory", font=('arial', 18), width=20, pady=10, highlightbackground="#202020", command=ShowInventory)
    btn_viewInv.grid(row=3, pady=20)
    btn_exit2 = Button(Home, text="Exit", font=('arial', 18), width=10, pady=10, highlightbackground="#202020", command=Exit2)
    btn_exit2.grid(row=10, pady=20)
    if privlevel == 2 or privlevel == 3:
        btn_changepass = Button(Home, text="Change Password", font=('arial', 18), width=20, pady=10, highlightbackground="#202020", command=ShowChangePass)
        btn_changepass.grid(row=4, pady=20)
    if privlevel == 1:
        btn_addnewuser = Button(Home, text="Users List", font=('arial', 18), width=20, pady=10, highlightbackground="#202020", command=ShowUserConfiguration)
        btn_addnewuser.grid(row=5, pady=20)
        btn_logs = Button(Home, text="View Log", font=('arial', 18), width=20, pady=10, highlightbackground="#202020", command=ShowLogs)
        btn_logs.grid(row=6, pady=20)
...
def EditForm():
    ...
    btn_add = Button(MidAddNew2, text="Update", font=('arial', 18), width=30, bg="#009ACD", highlightbackground="#202020", command=UpdateInventory)
...
def UpdateInventory():
    ...
    if BUILDING.get() == "" or ROOMNUMBER.get() == "" or LOCATION.get() == "":
        ...
        if BUILDING.get() == editSelect[1] and ROOMNUMBER.get() == str(editSelect[2]) and LOCATION.get() == editSelect[3]:
            ...
            cursor.execute("SELECT * FROM `inventory` WHERE `building` COLLATE NOCASE = ? AND `roomnumber` COLLATE NOCASE = ? AND `location` COLLATE NOCASE = ?", (BUILDING.get(), ROOMNUMBER.get(), LOCATION.get()))
...
def CheckOut():
    ...
    if AMOUNT.get() != 0:
        Database()
        cursor.execute("""SELECT count FROM inventory WHERE item_id = ? """, (checkSelect[0],))
        conn.commit
        fetch = cursor.fetchall()
        if len(fetch) == 0:
            ...
            if newAmount < 0:
                newAmount = 0
            cursor.execute("""UPDATE inventory SET count = ? WHERE item_id = ? """, (newAmount, checkSelect[0]))
            conn.commit()
            messagebox.showinfo('Check Out', 'Successfully updated inventory')
            addnewcheck.withdraw()
            cursor.execute("""SELECT * FROM inventory WHERE item_id = ?""", (checkSelect[0],))
            conn.commit()
            fetch = cursor.fetchone()
            partnum = fetch[7]
            business = fetch[4]
            if newAmount <= 10:
                cursor.execute("""SELECT * FROM inventory WHERE business = ? AND partnum = ? AND storage = ?""", (business, partnum, 'Yes'))
                conn.commit()
                fetch = cursor.fetchone()
                if fetch is None:
...
def ConfigurationForm():
    ...
    scrollbarx = Scrollbar(MidViewForm, orient=HORIZONTAL)
    scrollbary = Scrollbar(MidViewForm, orient=VERTICAL)
    tree2 = ttk.Treeview(MidViewForm, columns=("User_Id", "username", "privlevel"), selectmode="extended", height=100, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    ttk.Style().configure("Treeview", background="#CCBFB7", foreground="#000000", fieldbackground="#CCBFB7")
    tree2['show'] = 'headings'
...
def SearchAmp():
    if SEARCH.get() != "":
        ...
        Database()
        cursor.execute("SELECT * FROM `amp` WHERE `business` = ? AND `partdesc` LIKE ? OR `vendor` LIKE ?", ('inventory', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%'))
        fetch = cursor.fetchall()
        for data in fetch:
            tree.insert('', 'end', values=(data))
        cursor.close()
...
def Search():
     if SEARCH.get() != "":
        tree.delete(*tree.get_children())
        Database()
        cursor.execute("SELECT * FROM `inventory` WHERE `building` LIKE ? OR `roomnumber` LIKE ? OR `location` LIKE ? OR `vendor` LIKE ? OR `partdesc` LIKE ? OR `partnum` LIKE ? OR `category` LIKE ? or `subcategory` LIKE ?", ('%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%', '%'+str(SEARCH.get())+'%'))
        fetch = cursor.fetchall()
        for data in fetch:
            tree.insert('', 'end', values=(data))
        cursor.close()
...
btn_newlogin.pack(pady=10)
btn_exit = Button(root, text="Exit", font=('arial', 18), width=10, pady=10, command=Exit, highlightbackground="#202020")
btn_exit.pack()
lbl_email = Label(root, text="v1.1 by daniel.garcia@onqfinancial.com", font=('arial', 10), pady=60, fg="#BDBDBD", bg="#202020")
lbl_email.pack()
